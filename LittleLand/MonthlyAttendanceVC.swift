...
class MonthlyAttendanceVC: PagerController, PagerDataSource, PagerDelegate {
    ...
    var monthlyAttendanceArray:NSMutableArray = NSMutableArray()
    ...
    override func viewDidLoad() {
        ...
        lbl_Heder.text = ApiUtillity.sharedInstance.getLanguageData(key: "lbl_monthly_attendanced").uppercased()
        ...
        self.getMonthlyAttendanceReports()
    }
    ...
    //MARK:- All Method
    func getMonthlyAttendanceReports() {
        self.formatter.dateFormat = "yyyy-MM-dd"
        let type:String = (ApiUtillity.sharedInstance.getLoginType() == "teacher" ? "teacher" : "parent")
        let params = ["uid":ApiUtillity.sharedInstance.getUserData(key: "uid"), "class_id":selectedClassID, "action":"monthly_report", "type":type, "date":self.formatter.string(from: Date()), "lan":ApiUtillity.sharedInstance.getCurrentLanguageName()] as [String : Any]
        ...
        Alamofire.request(ApiUtillity.sharedInstance.API(Join: "attendance_report.php"), method: .post, parameters: params, encoding: URLEncoding.default).responseJSON { response in
            ...
            if let json = response.result.value {
                ...
                if (dict.value(forKey: "success") != nil) {
                    ...
                    self.monthlyAttendanceArray = success.mutableCopy() as! NSMutableArray
                    
                    if self.monthlyAttendanceArray.count < 3 {
                        self.tab_Width = UIScreen.main.bounds.size.width/CGFloat(self.monthlyAttendanceArray.count)
                        self.customizeTab()
                    }
                    ...
                    if self.monthlyAttendanceArray.count <= 0 {
    ...
    func numberOfTabs(_ pager: PagerController) -> Int {
        return monthlyAttendanceArray.count
    }
    ...
    func tabViewForIndex(_ index: Int, pager: PagerController) -> UIView {
        ...
        imageview_Student.kf.setImage(with: URL(string: ((monthlyAttendanceArray.object(at: index) as! NSDictionary).value(forKey: "profile_pic") as! String)))
        ...
        Student_Name.tag = 11
        Student_Name.text = (ApiUtillity.sharedInstance.getLoginType() == "teacher" ? "\(((monthlyAttendanceArray.object(at: index) as! NSDictionary).value(forKey: "fname") as! String)) \(((monthlyAttendanceArray.object(at: index) as! NSDictionary).value(forKey: "lname") as! String))" : "\(((monthlyAttendanceArray.object(at: index) as! NSDictionary).value(forKey: "fname") as! String))")
    ...
    func controllerForTabAtIndex(_ index: Int, pager: PagerController) -> UIViewController {
        let VC:MonthlyAttendanceSubVC = ApiUtillity.sharedInstance.getCurrentLanguageStoryboard().instantiateViewController(withIdentifier: "MonthlyAttendanceSubVC") as! MonthlyAttendanceSubVC
        VC.reportsArray = (((monthlyAttendanceArray.object(at: index) as! NSDictionary).value(forKey: "report") as! NSArray)).mutableCopy() as! NSMutableArray
        return VC
    }
